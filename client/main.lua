...
AddEventHandler("ZombieSync", function()
	...
	while true do
		...
		if #entitys < Config.SpawnZombie then
			...
			SetPedMoveRateOverride(entity, Config.ZombieMovementSpeed)
		...
		for i, entity in pairs(entitys) do
			for j, player in pairs(players) do
				...
				if distance <= Config.ZombieDetectRange then
					...
					TaskGoToEntity(entity, GetPlayerPed(player), -1, 0.0, Config.ZombieGoToSpeed, 1073741824, 0)
				end
			end
		end
	end
    ...
    while true do
        ...
        for i, entity in pairs(entitys) do
			...
			if IsPedDeadOrDying(entity, 1) == 1 then
				...
				if(Vdist(playerX, playerY, playerZ, pedX, pedY, pedZ) < 0.8)then
